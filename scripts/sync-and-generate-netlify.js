...
/**
 * 🔄 SINCRONIZADOR DE PROPOSTAS PARA NETLIFY
 * 
 * Este script sincroniza as propostas de src/data/clientes para pastanetilify/orçamento/clientes
 * e gera o index.html automaticamente
 */
...
// Configurações
const SOURCE_DIR = path.join(__dirname, '../src/data/clientes');
const TARGET_DIR = path.join(__dirname, '../pastanetilify/orçamento/clientes');
...
/**
 * Sincroniza arquivos HTML de src/data/clientes para pastanetilify
 */
function syncPropostas() {
    try {
        ...
        const clientes = fs.readdirSync(SOURCE_DIR);
        let syncCount = 0;

        console.log('🔄 Sincronizando propostas...');

        clientes.forEach(clienteDir => {
            const clientePath = path.join(SOURCE_DIR, clienteDir);
            
            if (fs.statSync(clientePath).isDirectory()) {
                const files = fs.readdirSync(clientePath);
                
                files.forEach(file => {
                    if (file.endsWith('.html')) {
                        const sourceFile = path.join(clientePath, file);
        ...
        console.log(`✅ ${syncCount} arquivos sincronizados`);
        return syncCount;
...
function scanPropostas() {
    try {
        if (!fs.existsSync(TARGET_DIR)) {
            console.log('❌ Pasta de propostas não encontrada:', TARGET_DIR);
            return [];
        }
 ...
 */
function main() {
    console.log('🔄 SINCRONIZADOR DE PROPOSTAS PARA NETLIFY');
    console.log('==========================================');
    
    // Sincronizar propostas
    const syncCount = syncPropostas();
    
    if (syncCount === 0) {
        console.log('❌ Nenhuma proposta encontrada para sincronizar');
        process.exit(1);
    }
    
    // Escanear propostas sincronizadas
    const propostas = scanPropostas();
    
    if (propostas.length === 0) {
        console.log('❌ Nenhuma proposta encontrada após sincronização');
        process.exit(1);
    }
...
module.exports = { syncPropostas, scanPropostas, generateIndexHTML };
